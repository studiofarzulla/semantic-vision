...
def main():
    ...
    output_file = OUTPUT_DIR / f"stochastic_baseline_{timestamp}.jsonl"
    ...
    # Stratified sample
    sampled_images = stratified_sample(original_images, args.n_images)
    print(f"[*] Stratified sample: {len(sampled_images)} images")
    ...
    if args.resume:
        ...
        existing = sorted(OUTPUT_DIR.glob("stochastic_baseline_*.jsonl"))
        if existing:
